...
router.get('/getPlans', async function (req, res) {
    ...
    const plans = await plan_1.Plan.find({}).populate('investments').exec();
    res.send(plans);
});
router.get('/getPlan/:planId', async function (req, res) {
    console.log('fetching plan');
    const _id = req.params.planId;
    let plan = await plan_1.Plan.findById(_id).exec();
    res.send(plan);
});
...
router.put('/updatePlan/:id/:name', async function (req, res) {
    ...
    const plan = await plan_1.Plan.findByIdAndUpdate(_id, { name }, options);
    res.send(plan);
});
router.put('/addInvestment/:id', async function (req, res) {
    ...
    const plan = await plan_1.Plan.findById(_id).populate("investments").exec();
...
router.delete('/deleteInvestment/:id/:planId', async function (req, res) {
    ...
    const plan = await plan_1.Plan.findById(planId).populate("investments").exec();
...
router.delete('/deletePlan/:planId', async function (req, res) {
    ...
    const _id = req.params.planId;
    let plan = await plan_1.Plan.findByIdAndDelete(_id);
    res.send(plan);
});
